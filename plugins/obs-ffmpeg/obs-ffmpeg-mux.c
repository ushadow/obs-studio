...
static bool ffmpeg_hls_mux_start(void *data)
{
	...
	stream->printable_path = path_str;
	stream_key = obs_service_get_key(service);
	...
	info("Writing to path '%s'...", stream->printable_path);
	return true;
}
...
static int deactivate(struct ffmpeg_muxer *stream, int code)
{
	...
	if (active(stream)) {
		...
		if (stream->printable_path)
			info("Output of file '%s' stopped",
			     stream->printable_path);
		else
...
static bool(struct ffmpeg_muxer *stream, struct encoder_packet *packet)
{
...
static void *replay_buffer_mux_thread(void *data)
{
	...
	if (stream->printable_path)
		info("Wrote replay buffer to '%s'", stream->printable_path);
	else
